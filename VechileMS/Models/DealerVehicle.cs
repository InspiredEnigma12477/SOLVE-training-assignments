...
namespace VechileMS.Models
{
	public class DealerVehicle : Vehicle
	{
		...
		{
			//return $"|{Id}\t|{base.ToString()}\t\t|{SellingMonthAndYear.ToString("MM/yyyy")}\t|{Number}\t|{OwnerDetails}\t|{ExpiryDate.ToString("dd/MM/yyyy")}\t|{ServicingHistory}\t\t|{IsAccidental}   \t|{IsInServicing}    \t|{string.Join(", ", Notes)}\t|";
			return $"\nVechile Id              : {Id}"
				+ base.ToString()
